// game-simulator.js — Simulador de batalla Epic Warriors
...
function renderSimulator() {
  ...
  // CSS del simulador standalone (extraído de v0.23)
  var simCSS = `
:root {
  --bg:      #07060a;
  --panel:   #110f1a;
...
/* ── CONTINGENT CARD ── */
.contingent-band {
  padding:8px 14px 0; margin-bottom:6px;
  border-radius:6px; border:1px solid var(--border);
  background:var(--panel);
  position:relative; overflow:hidden;
}
.contingent-band::before {
...
.btn-remove {
  ...
  cursor:pointer; font-family:'VT323',monospace; letter-spacing:.05em;
  transition:all .12s;
}
...
.troop-table th {
  font-size:.6rem; color:var(--dim); letter-spacing:.08em;
  text-align:right; padding:2px 5px 6px; font-weight:normal; white-space:nowrap;
}
...
.t-input {
  ...
  font-size:.88rem; padding:2px 4px; text-align:right; outline:none;
...
.btn-add {
  ...
  font-family:'VT323',monospace; font-size:.95rem; letter-spacing:.08em;
  transition:all .12s;
}
.btn-add.atk {
  background:rgba(255,112,80,.07); border:1px dashed rgba(255,112,80,.35); color:var(--atk);
}
.btn-add.atk:hover { background:rgba(255,112,80,.14); }
.btn-add.def {
  background:rgba(96,176,255,.07); border:1px dashed rgba(96,176,255,.35); color:var(--def);
}
...
.btn-sim {
  flex:1; background:linear-gradient(135deg,var(--gold),#b06800);
  color:#000; border:none; padding:13px 0;
  font-family:'VT323',monospace; font-size:1.35rem;
  border-radius:5px; cursor:pointer; letter-spacing:.07em;
  box-shadow:0 4px 20px rgba(240,192,64,.2);
  transition:all .15s;
}
...
/* if only 1 per side, span full */
.res-contingents.single-atk .res-contingent.atk,
.res-contingents.single-def .res-contingent.def { grid-column:1/-1; }
...
@media (max-width:900px) {
  ...
  .col-log { width:100%; }
  .res-contingents { grid-template-columns:1fr; }
}
  `;

  // JS del simulador standalone (extraído de v0.23) con datos del juego inyectados
  var simJS_template = `
// ============================================================
// DATOS DE TROPAS Y CRIATURAS
// ============================================================
const TROOPS = {
  aldeano:    { name:'Aldeano',    icon:'👤', attackChance:8,  hp:10,  attacksPerTurn:1, damage:2,  defense:10, dexterity:5  },
  soldado:    { name:'Soldado',    icon:'⚔️', attackChance:12, hp:25,  attacksPerTurn:1, damage:8,  defense:14, dexterity:8  },
  mago:       { name:'Mago',       icon:'🧙', attackChance:15, hp:15,  attacksPerTurn:2, damage:12, defense:10, dexterity:12 },
  druida:     { name:'Druida',     icon:'🌿', attackChance:14, hp:20,  attacksPerTurn:1, damage:6,  defense:12, dexterity:10 },
  explorador: { name:'Explorador', icon:'🏹', attackChance:16, hp:12,  attacksPerTurn:2, damage:5,  defense:11, dexterity:15 },
  asesino:    { name:'Asesino',    icon:'🗡️', attackChance:18, hp:18,  attacksPerTurn:2, damage:14, defense:12, dexterity:18 },
  paladin:    { name:'Paladín',    icon:'🛡️', attackChance:10, hp:40,  attacksPerTurn:1, damage:10, defense:18, dexterity:6  },
  chaman:     { name:'Chamán',     icon:'🔮', attackChance:13, hp:22,  attacksPerTurn:2, damage:10, defense:11, dexterity:11 },
  invocador:  { name:'Invocador',  icon:'🧿', attackChance:11, hp:20,  attacksPerTurn:1, damage:6,  defense:12, dexterity:9  },
  ballestero: { name:'Ballestero', icon:'🎯', attackChance:14, hp:14,  attacksPerTurn:2, damage:7,  defense:11, dexterity:13 },
};

const CREATURES = {
  orco:     { name:'Orco',     icon:'👹', attackChance:10, hp:30,  attacksPerTurn:1, damage:10, defense:12, dexterity:6  },
  hada:     { name:'Hada',     icon:'🧚', attackChance:14, hp:20,  attacksPerTurn:2, damage:8,  defense:10, dexterity:15 },
  golem:    { name:'Gólem',    icon:'🗿', attackChance:8,  hp:80,  attacksPerTurn:1, damage:18, defense:18, dexterity:4  },
  espectro: { name:'Espectro', icon:'👻', attackChance:16, hp:50,  attacksPerTurn:1, damage:22, defense:8,  dexterity:18 },
  grifo:    { name:'Grifo',    icon:'🦅', attackChance:14, hp:120, attacksPerTurn:2, damage:28, defense:14, dexterity:16 },
  hidra:    { name:'Hidra',    icon:'🐉', attackChance:12, hp:180, attacksPerTurn:3, damage:20, defense:16, dexterity:10 },
  fenix:    { name:'Fénix',    icon:'🔥', attackChance:16, hp:200, attacksPerTurn:2, damage:45, defense:12, dexterity:20 },
  behemot:  { name:'Behemot',  icon:'🦏', attackChance:10, hp:350, attacksPerTurn:1, damage:55, defense:22, dexterity:6  },
  dragon:   { name:'Dragón',   icon:'🐲', attackChance:18, hp:500, attacksPerTurn:3, damage:80, defense:20, dexterity:14 },
  arconte:  { name:'Arconte',  icon:'👼', attackChance:20, hp:400, attacksPerTurn:2, damage:70, defense:25, dexterity:18 },
};

// ============================================================
// ESTADO — lista de contingentes
// ============================================================
let contingents = [];
let nextId = 1;
...
return;
  ...
  // Ordenar: primero atacantes, luego defensores
  const sorted = [
...contingents.filter(c => c.side === 'atk'),
...contingents.filter(c => c.side === 'def'),
  ];
...
// wallBonus: solo para defensores
function buildGroups(comp, side, wallBonus, contingentId) {
  const groups = [];
  let gid = 1;
...
const effDef = isCreature ? data.defense : data.defense + arm + (side === 'def' ? wallBonus : 0);
divGroups(qty).forEach(size => {
  groups.push({
    gid: gid++, side, key, contingentId,
    name: data.name, icon: data.icon,
    stats: {
      attackChance: data.attackChance, attacksPerTurn: data.attacksPerTurn,
      dexterity: data.dexterity, hp: effHP, damage: effDmg, defense: effDef,
    },
...
function runSimulation() {
  ...
  let   wallHP    = wallLvl > 0 ? wallLvl * 500 : 0;
  ...
  // Recoger datos actuales de los inputs (actualizar nombre)
  contingents.forEach(c => {
const el = document.getElementById('name_' + c.id);
...
atk = atk.concat(buildGroups(comp, 'atk', 0, c.id));
  });
  defConts.forEach(c => {
const comp = getContingentComp(c.id);
def = def.concat(buildGroups(comp, 'def', wallBonus, c.id));
  });

  const hasAtk = atk.length > 0;
  const hasDef = def.length > 0;
  if (!hasAtk) { addLog('⚠️ Los atacantes no tienen tropas.', 'lm'); return; }
  if (!hasDef) { addLog('⚠️ Los defensores no tienen tropas.', 'lm'); return; }

  // Snapshots para la tarjeta de resultado
...
if (turn > 300) { addLog('⚠️ Batalla detenida — límite de turnos alcanzado.', 'lm'); break; }
  ...
  // Detectar muralla resistida
  if (wallHP > 0) wallResisted = true;
...
...def.filter(g => g.count > 0).map(g => ({ ...g, enemyArmy: atk })),
  ...
  // Iniciativa por destreza (mayor va primero), desempate aleatorio
  all.sort((a, b) => {
...
for (let i = 0; i < group.stats.attacksPerTurn; i++) {
  ...
  const tIcon = target.side === 'atk' ? '⚔️' : '🛡️';
...
return m;
  ...
  // Tasa de recuperación aleatoria (10-30%) por contingente
  const recRates = {};
  [...atkConts, ...defConts].forEach(c => { recRates[c.id] = 0.10 + Math.random() * 0.20; });

  // Si muralla resistió: no hay bajas entre defensores, no hay recuperación para atacantes
...
Object.keys(st[cid] || {}).forEach(n => {
  const i = st[cid][n].count;
  const f = (surv[cid] && surv[cid][n]) || 0;
  const d = i - f;
...
<th class="rc-tot">Total</th>
  ...
  const bannerTxt = winSide === 'atk' ? '⚔️ VICTORIA DEL BANDO ATACANTE'
: winSide === 'def' ? (wallResisted ? '🏰 VICTORIA DEL DEFENSOR — MURALLA INTACTA' : '🛡️ VICTORIA DEL BANDO DEFENSOR')
: '⚔️ EMPATE — Ambos ejércitos eliminados';

  // Tarjetas de contingentes atacantes
...
// ============================================================
// INIT — 1 atacante + 1 defensor por defecto
// ============================================================
addContingent('atk');
addContingent('def');
updateWallInfo();
`;
  ...
  // Reemplazar las constantes TROOPS y CREATURES con los datos del juego
  var simJS = simJS_template
    .replace(/^const TROOPS\s*=\s*\{[\s\S]*?\};\s*\n/m, '')
    .replace(/^const CREATURES\s*=\s*\{[\s\S]*?\};\s*\n/m, '');

  // Inyectar datos del juego al principio
  ...
  var fullHTML = '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">' +
    ...
    '<style>' + simCSS + '</style>' +
    '<style>body{margin:0;}' +
    '.workspace{max-width:100%;padding:10px;}' +
    '.col-log{width:400px;}' +
    '</style>' +
    '</head><body>' +
    '<div class="workspace">' +
    '<div class="col-armies">' +
    '<div class="wall-panel"><label>🏰 Nivel de Muralla (Defensor):</label>' +
    '<input class="wall-input" type="number" id="wallLevel" value="0" min="0" max="20" oninput="updateWallInfo()">' +
    '<div class="wall-info" id="wallInfo">Sin muralla</div></div>' +
    '<div id="contingentsArea"></div>' +
    '<div class="add-row">' +
    '<button class="btn-add atk" onclick="addContingent(\'atk\')">⚔ + Añadir Atacante</button>' +
    '<button class="btn-add def" onclick="addContingent(\'def\')">🛡 + Añadir Defensor</button>' +
    '</div>' +
    '<div class="btn-row">' +
    '<button class="btn-sim" onclick="runSimulation()">⚔️ INICIAR BATALLA</button>' +
    '<button class="btn-clr" onclick="clearLog()">Limpiar</button>' +
    '</div></div>' +
    '<div class="col-log">' +
    '<div class="log-panel"><div class="log-head"><span>📜 Registro</span>' +
    '<span id="logStats" style="font-size:.72rem;color:var(--dim);"></span></div>' +
    '<div class="log-body" id="battleLog"><div class="empty-log">Configura y pulsa Iniciar</div></div>' +
    '</div></div></div>' +
    '<script>' + dataJS + simJS + '<\/script>' +
    '</body></html>';

  // Inyectar como iframe vía blob
