### v1.95 — Fix construcción + aldeanos + inicio auditoría OGame puro

---

#### BUGS CORREGIDOS

- **[REGLA v1.95 — CRÍTICO] Check de cola en RPCs:** Siempre leer de la columna, nunca del state devuelto por `secure_village_tick`. Patrón correcto:
```sql
SELECT build_queue INTO v_col_bq FROM villages WHERE id = p_village_id;
IF v_col_bq IS NOT NULL AND v_col_bq != 'null'::jsonb AND v_col_bq != '[]'::jsonb THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Cola ocupada');
END IF;
```

- **[REGLA v1.95 — CRÍTICO] Edificios de aldeanos en `secure_village_tick`:**
  - `v_ald_max` → **`barracas`** → `FLOOR(100 * POWER(1.4, nivel-1))` (= `getBarracksCapacity` en JS)
  - `v_ald_interval` → **`reclutamiento`** → `(600/3600) * (1-(nivel-1)*0.01)` (= `getAldeanosIntervalMs` en JS)
  - El edificio `casa` NO existe en el juego. Nunca usarlo.

- **[REGLA v1.95 — CRÍTICO] `completed_builds` en `secure_village_tick`:** Inicializar como `'[]'::jsonb` en DECLARE. Nunca `current_setting()`. Completación inline: si `finish_at <= NOW()` → aplicar nivel → rellenar `completed_builds` → `build_queue = NULL`.

- **[REGLA v1.95 — CRÍTICO] UPDATE en RPCs nunca fosiliza colas en JSONB `state`:** Usar `state - 'build_queue'`, `state - 'summoning_queue'`, etc.

- **[REGLA v1.95] `switchVillage`:** Llamar `syncVillageResourcesFromServer()` tras `tick()` y resetear `_lastResourceSync = Date.now()`.

---

#### AUDITORÍA MODELO OGAME PURO — EN PROGRESO

**Estado:** 15 bugs identificados, ninguno aplicado aún. Ver `AUDITORIA_OGAME_v195.md`.
...
**Lo que hay que eliminar (bugs pendientes):**
| Bug | Archivo | Descripción |
|---|---|---|
| BUG-01 | index.html | Sistema animación `uiShown/uiTarget/ensureUiAnim` — eliminar |
| BUG-02 | index.html | Generación local de aldeanos en `tick()` — eliminar |
| BUG-03 | game-engine.js | `calcRes` interpola recursos con tiempo — simplificar a devolver `vs.resources` |
| BUG-04 | game-ui.js | `snapshotResources` sobreescribe resources con `calcRes` — eliminar ese path |
| BUG-05 | index.html | Sync periódico 60s — eliminar |
| BUG-06 | game-engine.js | `startMission` optimistic update local — eliminar |
| BUG-07 | game-engine.js | `startMission` aplica state sin merge de colas — verificar/fix |
| BUG-08 | game-ui.js | `startBuild` aplica estado parcial — aplicar state completo |
| BUG-09 | game-ui.js | `cancelBuild` aplica estado parcial — aplicar state completo |
| BUG-10 | game-troops.js | `startSummoning` aplica estado parcial — aplicar state completo |
| BUG-11 | game-troops.js | `startRecruitment` aplica estado parcial — aplicar state completo |
| BUG-12 | game-ui.js | `syncVillageResourcesFromServer` renombrar a `triggerServerTick` |
| BUG-13 | index.html | `_ogameSyncRunning` puede quedar huérfano en catch |
| BUG-14 | index.html/ui | `saveVillage` llama `snapshotResources` — ok tras simplificación |
| BUG-15 | index.html | `tick()` usa `calcRes` para `uiTarget` — desaparece al eliminar BUG-01 |

**Primer paso en el nuevo chat:** ejecutar SQL antes de tocar JS:
```sql
SELECT prosrc FROM pg_proc
WHERE proname = 'launch_mission_secure_v2'
AND pg_get_function_identity_arguments(oid) LIKE '%duration%';
```
Verificar que devuelve `build_queue`, `training_queue`, `summoning_queue` — necesario para BUG-07.

**Lo que NO se toca:**
- `tick()` como loop de 1s (countdowns y resolución de misiones)
- `resolveMissions` (ataques, espionajes)
- `flushVillage` / `saveVillage` (persiste colas)
- `getProd()` (producción/h en UI — estático)
- Admin Global Tick

**Reglas nuevas:** #38, #39, #40, #41, #42 — ver ARQUITECTURA.md.

**Archivos a subir en el nuevo chat:** `index.html`, `game-engine.js`, `game-ui.js`, `game-troops.js` + este REFERENCIA.md + ARQUITECTURA.md + AUDITORIA_OGAME_v195.md.

### v1.93 — Fix start_training_secure (producción inline eliminada)
- **[REGLA v1.93 — CRÍTICO]** `start_training_secure` ya NO calcula producción inline. Llama `PERFORM secure_village_tick` al inicio igual que `start_build_secure`. Si se modifica este RPC, NO reintroducir las variables `prod_*`, `cur_*`, `bon_*` ni el cap de almacén manual — todo eso queda obsoleto.
- **[REGLA v1.93]** Los tres RPCs `start_*_secure` (build, training, summoning) siguen el mismo patrón: tick → leer estado fresco → validar → descontar → encolar. Nunca duplicar lógica de producción dentro del RPC.
- DT-11, DT-12, DT-13, DT-14: todos cerrados en esta sesión.

### v1.92 — Flag _resourcesFromServer + fórmula aldeanos
- **[REGLA v1.92 — CRÍTICO] Flag `_resourcesFromServer`:** Antes de llamar `flushVillage()` tras una RPC que devuelve resources, poner `vs._resourcesFromServer = true`. Esto evita que `snapshotResources` sobreescriba el valor correcto del servidor con interpolación local. Funciones afectadas: `startBuild`, `cancelBuild`, `startSummoning`, `startRecruitment`, `cancelTrainingQueue`.
- **[REGLA v1.92 — CRÍTICO] Fórmula intervalo aldeanos (sincronizar siempre):** `600000 * (1 - (nivel-1) * 0.01)` ms. Debe estar igual en `getAldeanosIntervalMs` (game-constants.js) Y en `secure_village_tick` (Supabase). Si se cambia en uno, cambiar en el otro en la misma sesión.
